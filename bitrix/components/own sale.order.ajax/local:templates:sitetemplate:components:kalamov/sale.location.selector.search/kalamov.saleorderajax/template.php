...
$isAjax =  isset($_SERVER['HTTP_X_REQUESTED_WITH']) || (isset($_REQUEST['AJAX_CALL']) && $_REQUEST['AJAX_CALL']=='Y');
...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls bx-sls--search<?if (strlen($arResult['MODE_CLASSES'])):?> <?=$arResult['MODE_CLASSES']?><?endif?>">
		...
		<div class="bx-ui-sls-input-block">
			<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" value="<?=$arResult['VALUE']?>" class="form-control" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?>" />
